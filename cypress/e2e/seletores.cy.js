...
describe('Seletores avançados com cypress', () => {
  ...
  it('Seleciona o elemento com a classe pai do elemento com a classe filho', () => {
    ...
    cy.get('.filho-4').parent('.pai').should('have.attr', 'data-test', 'div-pai')
  });

  it('Seleciona o elemento com um valor em um select', () => {
    cy.get('[name="opcao"]').select('Muito').should('contain', 'Muito') //Validação pelo texto
    cy.get('[name="opcao"]').select('muito').should('have.value', 'muito') //Validação pelo value
